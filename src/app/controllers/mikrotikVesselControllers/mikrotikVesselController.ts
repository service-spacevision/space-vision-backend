...
import { getVesselProfilesAndServers_func } from './functions/getVesselProfilesAndServers';
...
export class MikrotikVesselController {
  ...
  static async getVesselProfilesAndServers(ctx: CustomContext) {
    try {
      const { query } = ctx;
      const user = ctx.user!;

      const result = await getVesselProfilesAndServers_func({
        reqObject: { user },
        query: {
          vesselId: query.vesselId as string,
        },
      });
      ...
      return {
        success: false,
        message:
          'Internal server error while fetching vessel profiles and servers',
      };
    }
  }
}
