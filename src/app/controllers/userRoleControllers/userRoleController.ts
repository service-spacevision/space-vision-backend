...
export class UserRoleController {
    ...
    try {
      ...
      const result = await createUserRole_func({
        data: {
          ...
          // user: user,
          organizationName:
            (body as any)?.organizationName ?? user?.organizationName,
        },
        user: user as any,
      });
  ...
  static async getRoles(ctx: CustomContext) {
    try {
      ...
      const searchQuery = query?.search ? String(query.search) : '';
      const result = await getUserRoles_func({ includeInactive, pagination, searchQuery });
