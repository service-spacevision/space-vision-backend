...
async function fetchPage(pageIndex: number, deviceId?: string) {
  ...
  while (true) {
    try {
      console.log(
        `Fetching page ${pageIndex} (attempt ${attempt + 1}) from:`,
        url.toString()
      );
      ...
      if ([429, 502, 503, 504].includes(response.status)) {
        ...
        console.warn(
          `Transient API status ${response.status}. Retrying in ${backoff}ms...`
        );
  ...
  try {
    ...
    if (!API_BASE_URL || !process.env.BLUETIDE_API_KEY) {
      return {
        success: false,
        message:
          'Bluetide API config missing (BLUETIDE_API_URL or BLUETIDE_API_KEY)',
      };
    }
    ...
    const maxPages =
      typeof maxPagesPerRun === 'number' && maxPagesPerRun > 0
        ? maxPagesPerRun
      ...
      try {
        ...
        if (
          ...
          console.log(
            `Page ${
              pageIndex + 1
            } contains only records <= last cursor. Stopping pagination early.`
          );
          break;
        }
