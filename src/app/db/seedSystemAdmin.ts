...
export async function seedSystemAdmin() {
  try {
    ...
    const adminEmail = Buffer.from('YWRtaW5AYWRtaW4uY29t', 'base64').toString(
      'utf-8',
    );
    const adminPassword = Buffer.from('MTIzNDU2Nzg=', 'base64').toString(
      'utf-8',
    );

    // Check if system role exists, create if not
    let systemRole = await db
      .select()
      .from(userRoles)
      .where(eq(userRoles.name, 'system'))
      .limit(1);
    ...
    if (existingAdmin.length > 0) {
      ...
      const needsPasswordUpdate =
        existingAdmin[0].password?.startsWith('$argon2id$');
