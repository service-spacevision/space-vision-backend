...
const mikrotikUsageRoute = new Elysia({ prefix: '/api/mikrotik-usage' })
  ...
  .post('/crew-login/crew', MikrotikUsageController.crewLogin, {
    body: t.Object({
      username: t.String({
  ...
  // System login endpoint - no auth required
  .post('/crew-login/system', MikrotikUsageController.systemLogin, {
    body: t.Object({
      username: t.String({
    ...
    detail: {
      tags: ['Mikrotik Usage'],
      summary: 'System login to access vessel usage data',
      description: 'Authenticate system user and return vessel usage data',
    },
  });
