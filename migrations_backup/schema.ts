...
export const mikrotikUsageSession = pgTable('mikrotik_usage_session', {
  ...
  mac: text(),
  uptime: text(),
  rxMb: bigint('rx_mb', { mode: 'number' }).default(0).notNull(),
  txMb: bigint('tx_mb', { mode: 'number' }).default(0).notNull(),
...
export const mikrotikUsageAlltime = pgTable(
  'mikrotik_usage_alltime',
  {
    ...
    rxMb: bigint('rx_mb', { mode: 'number' }).default(0).notNull(),
    txMb: bigint('tx_mb', { mode: 'number' }).default(0).notNull(),
    totalAllowedMb: bigint('total_allowed_mb', { mode: 'number' })
      .default(5000)
      .notNull(),
    percentageUsed: numeric('percentage_used', { precision: 5, scale: 1 })
      .default('0.0')
  ...
  (table) => ({
    unqVesselUsername: unique('unq_vessel_username').on(
      table.vesselName,
      table.username
    ),
  })
...
export const bluetideTelemetry = pgTable(
  'bluetide_telemetry',
  {
    id: serial().primaryKey().notNull(),
    ...
    secondsUntilSwupdateRebootPossible: integer(
  ...
  (table) => [
    uniqueIndex('bluetide_telemetry_device_timestamp_unique').using(
      'btree',
      table.deviceId.asc().nullsLast().op('text_ops'),
      table.timestamp.asc().nullsLast().op('text_ops')
    ),
    index('idx_bluetide_telemetry_account').using(
      'btree',
      table.accountNumber.asc().nullsLast().op('text_ops')
    ),
    index('idx_bluetide_telemetry_device_id').using(
      'btree',
      table.deviceId.asc().nullsLast().op('text_ops')
    ),
    index('idx_bluetide_telemetry_device_timestamp').using(
      'btree',
      table.deviceId.asc().nullsLast().op('text_ops'),
      table.timestamp.desc().nullsFirst().op('text_ops')
    ),
    index('idx_bluetide_telemetry_location').using(
      'gist',
      sql`ll_to_earth(latitude, longitude)`
    ),
    index('idx_bluetide_telemetry_timestamp').using(
      'btree',
...
export const mikrotikVessels = pgTable(
  'mikrotik_vessels',
  {
    id: serial().primaryKey().notNull(),
    vesselName: text('vessel_name').notNull(),
  ...
  (table) => [
    unique('mikrotik_vessels_name_ip_port_unique').on(
      table.vesselName,
      table.routerIp,
      table.apiPort
    ),
...
export const telephonyDids = pgTable(
  ...
  (table) => [unique('telephony_dids_number_unique').on(table.number)]
);
...
export const vesselGroups = pgTable(
  'vessel_groups',
  {
    id: serial().primaryKey().notNull(),
  ...
  (table) => [unique('vessel_groups_group_name_unique').on(table.groupName)]
);
...
export const syncState = pgTable(
  ...
  (table) => [
    uniqueIndex('sync_state_source_partition_unique').using(
      'btree',
      table.source.asc().nullsLast().op('text_ops'),
      table.partitionKey.asc().nullsLast().op('text_ops')
    ),
  ]
);
...
export const rolesPermission = pgTable(
  'roles_permission',
  {
    id: serial().primaryKey().notNull(),
  ...
  (table) => [unique('roles_permission_roleId_unique').on(table.roleId)]
);

export const starlinkUsage = pgTable(
  'starlink_usage',
  {
    id: serial().notNull(),
  ...
  (table) => [
    primaryKey({
      columns: [table.dateKey, table.kitNumber],
      name: 'starlink_usage_date_key_kit_number_pk',
    }),
  ]
);
export const starlinkUsageStatsMv = pgMaterializedView(
  'starlink_usage_stats_mv',
  {
    ...
    // You can use { mode: "bigint" } if numbers are exceeding js number limitations
    systemTotalVesselGroups: bigint('system_total_vessel_groups', {
      mode: 'number',
    }),
    systemTotalLifetimeUsageGb: real('system_total_lifetime_usage_gb'),
    // You can use { mode: "bigint" } if numbers are exceeding js number limitations
    systemTotalActiveKits: bigint('system_total_active_kits', {
      mode: 'number',
    }),
...
export const starlinkSystemSummaryMv = pgMaterializedView(
  'starlink_system_summary_mv',
  {
    ...
    // You can use { mode: "bigint" } if numbers are exceeding js number limitations
    kitsWithPublicIpEnabled: bigint('kits_with_public_ip_enabled', {
      mode: 'number',
    }),
    lastUpdated: timestamp('last_updated', {
      withTimezone: true,
